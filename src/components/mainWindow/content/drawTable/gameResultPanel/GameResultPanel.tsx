...
const GameResultPanel: React.FC = () => {
  const { gameState, currentWord, scoreAddUser } = useSelector(
  ...
  const [cssOpacity, setCssOpacity] = useState<React.CSSProperties>({
    opacity: 1,
  });
  ...
  useEffect(() => {
    setShowForDrawingUser(
      currentStartUser === sessionStorage.getItem(NICK_IN_STORAGE)
    );
    setShowForGuessed(
      scoreAddUser.userGuessed === sessionStorage.getItem(NICK_IN_STORAGE)
    );
    ...
    <>
      {showForDrawingUser && (
        <div className={styles.boxWrapPanelResult} style={cssOpacity}>
          {isWordGuessed && (
            <p className={styles.text}>Вы смогли нарисовать слово</p>
          )}
      ...
      {!showForDrawingUser && !showForGuessedUser && (
        <div className={styles.boxWrapPanelResult} style={cssOpacity}>
          <p className={styles.text}>Загаданное слово: {currentWord}</p>
          {!isWordGuessed && (
            <p className={styles.text}>Никто не угадал слово в этом раунде</p>
          )}
          {isWordGuessed && (
            <p className={styles.text}>
              Слово угадал: {scoreAddUser.userGuessed}
            </p>
          )}
          <p className={styles.text}>Ожидайте когда ведущий передаст ход</p>
        </div>
