...
function JoinRoom() {
  ...
  const handlePressEnter = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") await handleJoinRoom();
  };
  ...
  return (
    <div className={style.section + " " + style.joinRoom}>
      <div className={style.widget}>
        ...
        {isErrorRoom && (
          <div className={style.errorMessage}>Такой комнаты не существует</div>
        )}
        <input
        ...
        <button className={`${style.button} btn`} onClick={handleJoinRoom}>
          Присоединиться
        </button>
      </div>
    </div>
